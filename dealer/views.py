...
def payment_report(request):
    ...
    ist = pytz_timezone('Asia/Kolkata')
    current_date = timezone.now().astimezone(ist).date()
    collections = DealerCollectionReport.objects.filter(date=current_date,dealer=dealer_obj).all()
    print(collections)
    from_dealer_total = DealerCollectionReport.objects.filter(date=current_date,from_or_to='received',dealer=dealer_obj).aggregate(from_dealer=Sum('amount'))
    to_dealer_total = DealerCollectionReport.objects.filter(date=current_date,from_or_to='paid',dealer=dealer_obj).aggregate(to_dealer=Sum('amount'))
    ...
    profit_or_loss_dealer =  to_dealer_amount - from_dealer_amount
    if request.method == 'POST':
        from_date = request.POST.get('from-date')
        to_date = request.POST.get('to-date')
        from_or_to = request.POST.get('from-to')
        print(from_date,to_date,from_or_to)
        if from_or_to != 'all' and from_or_to == 'received':
            collections = DealerCollectionReport.objects.filter(date__range=[from_date, to_date],dealer=dealer_obj,from_or_to='paid').all()
            ...
            return render(request, 'dealer/payment_report.html', context)
        elif from_or_to != 'all' and from_or_to == 'paid':
            collections = DealerCollectionReport.objects.filter(date__range=[from_date, to_date],dealer=dealer_obj,from_or_to='received').all()
            from_dealer_amount = collections.aggregate(amount=Sum('amount'))
            profit_or_loss = from_dealer_amount['amount'] if from_dealer_amount['amount'] else 0
            context = {
                'collections': collections,
            ...
            collections = DealerCollectionReport.objects.filter(date__range=[from_date, to_date],dealer=dealer_obj).all()
            print(collections)
            from_dealer_total = DealerCollectionReport.objects.filter(date__range=[from_date, to_date],from_or_to='received',dealer=dealer_obj).aggregate(from_dealer=Sum('amount'))
            to_dealer_total = DealerCollectionReport.objects.filter(date__range=[from_date, to_date],from_or_to='paid',dealer=dealer_obj).aggregate(to_dealer=Sum('amount'))
            from_dealer_amount = from_dealer_total['from_dealer'] if from_dealer_total['from_dealer'] else 0
            to_dealer_amount = to_dealer_total['to_dealer'] if to_dealer_total['to_dealer'] else 0
            profit_or_loss_dealer =  to_dealer_amount - from_dealer_amount
            context = {
                'collections': collections,
                'profit_or_loss': profit_or_loss_dealer,
                'from_or_to' : from_or_to,
                'selected_from' : from_date,
                'selected_to' : to_date
            }
