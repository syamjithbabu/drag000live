        ...
        if select_agent != 'all' and select_dealer == 'all':
            ...
            if select_time != 'all':
                ...
                if lsk != 'all':
                    ...
                    agent_bills_totals = agent_games.aggregate(
                        total_c_amount=Sum('c_amount'),
                        total_d_amount=Sum('d_amount'),
                    ...
                    dealer_bills_totals = dealer_games.aggregate(
                        total_c_amount_admin=Sum('c_amount_admin'),
                        total_d_amount_admin=Sum('d_amount_admin'),
                        total_count=Sum('count')
                    )
            ...
            if select_time != 'all':
                ...
                if lsk != 'all':
                    ...
                    bills = Bill.objects.filter(date__range=[from_date, to_date]).distinct().order_by('-id')
                    agent_bills_totals = agent_games.aggregate(
                        total_c_amount=Sum('c_amount'),
                        total_d_amount=Sum('d_amount'),
                        total_count=Sum('count')
    ...
    if request.method == 'POST':
        ...
        if select_agent != 'all' and select_dealer == 'all':
            ...
            if select_time != 'all':
                ...
                total_agent_winning = agent_winnings.aggregate(agent_total=Sum('total'))['agent_total'] or 0

                dealer_winnings = Winning.objects.filter(date__range=[from_date, to_date],time=select_time, dealer__isnull=False)
                total_dealer_winning = dealer_winnings.aggregate(dealer_total=Sum('total_admin'))['dealer_total'] or 0

                total_winning = total_agent_winning + total_dealer_winning
            ...
            if select_time != 'all':
                ...
                dealer_winnings = Winning.objects.filter(Q(dealer=dealer_instance),date__range=[from_date, to_date],time=select_time)
                total_dealer_winning = dealer_winnings.aggregate(dealer_total=Sum('total_admin'))['dealer_total'] or 0

                total_winning = total_dealer_winning
                ...
                dealer_winnings = Winning.objects.filter(Q(dealer=dealer_instance),date__range=[from_date, to_date])
                total_dealer_winning = dealer_winnings.aggregate(dealer_total=Sum('total_admin'))['dealer_total'] or 0
            ...
            if select_time != 'all':
                ...
                bills = Bill.objects.filter(date__range=[from_date, to_date])
                paginator = Paginator(bills, 15)
                page = request.POST.get('page', 1)
                try:
                    combined_bills = paginator.page(page)
        ...
        bills = Bill.objects.filter(date=current_date)
        paginator = Paginator(bills, 15)
        page = request.GET.get('page', 1)
        try:
            combined_bills = paginator.page(page)
        ...
        total_winning = total_agent_winning + total_dealer_winning
        agent_total_c_amount = bills.aggregate(total_c_amount=Sum('agent_games__c_amount'))['total_c_amount'] or 0
        dealer_total_c_amount = bills.aggregate(total_c_amount=Sum('dealer_games__c_amount_admin'))['total_c_amount'] or 0
        total_c_amount = agent_total_c_amount + dealer_total_c_amount
        total_balance = float(total_c_amount) - float(total_winning)
        select_agent = 'all'
        select_time = 'all'
...
def delete_collection(request,id):
    delete_collect = CollectionReport.objects.get(id=id)
    delete_collect.delete()
    return redirect('adminapp:payment_report')
    ...
    if request.method == 'POST':
        ...
        if select_agent != 'all':
            ...
            for date in date_range:
                agent_instance = Agent.objects.get(id=select_agent)
                agent_games = AgentGame.objects.filter(date=date, agent=select_agent)
                dealer_games = DealerGame.objects.filter(date=date, agent=select_agent)
                collection = CollectionReport.objects.filter(date=date, agent=select_agent)
                agent_winning = Winning.objects.filter(Q(agent=select_agent),date=date).aggregate(total_winning=Sum('total'))['total_winning'] or 0
                dealer_winning = Winning.objects.filter(Q(dealer__agent=select_agent),date=date).aggregate(total_winning=Sum('total_admin'))['total_winning'] or 0
                winning = agent_winning + dealer_winning
                agent_total_d_amount = agent_games.aggregate(agent_total_d_amount=Sum('c_amount'))['agent_total_d_amount'] or 0
                dealer_total_d_amount = dealer_games.aggregate(dealer_total_d_amount=Sum('c_amount'))['dealer_total_d_amount'] or 0
                from_agent = collection.filter(from_or_to='received').aggregate(collection_amount=Sum('amount'))['collection_amount'] or 0
